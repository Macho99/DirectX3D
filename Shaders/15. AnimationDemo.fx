...
matrix GetAnimationMatrix(VertexTextureNormalTangentBlend input)
{
	...
	int curFrame = Keyframes.curFrame;
	...
	for (int i = 0; i < 4; i++)
	{
		c0 = TransformMap.Load(int4((indices[i] * 4 + 0), curFrame, animIndex, 0));
		c1 = TransformMap.Load(int4((indices[i] * 4 + 1), curFrame, animIndex, 0));
		c2 = TransformMap.Load(int4((indices[i] * 4 + 2), curFrame, animIndex, 0));
		c3 = TransformMap.Load(int4((indices[i] * 4 + 3), curFrame, animIndex, 0));
		...
		matrix result = lerp(cur, next, ratio);
